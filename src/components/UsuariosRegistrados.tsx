...
export default function UsuariosRegistrados({ users, totalUsers, currentPage, limit }: Props) {
  ...
  const handleDelete = async (id: number) => {
    const userConfirmed = confirm("¿Estás seguro de eliminar el usuario?");
    if (userConfirmed) {
      try {
        const result = await await deleteUser(id);
                    ...
                    <TableRow key={row.id} className="dark:bg-slate-800 dark:text-white">
                      ...
                      <TableCell className=''>
                        <div className="flex justify-center align-items-center space-x-8">
                          <Link href={`/usuarios/view/${row.id}`} aria-label='Editar Usuario'
                          ...
                          <button
                            onClick={() => handleDelete(row.id)} aria-label='Eliminar Usuario'
