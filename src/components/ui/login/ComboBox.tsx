      ...
      <Popover open={open} onOpenChange={setOpen}>
        <PopoverTrigger asChild>
          <Button
            ...
            role="combobox"
            aria-expanded={open}
            className="text-md py-7 w-full justify-between border-gray-300 hover:border-gray-400 focus:ring-2 focus:ring-blue-300 transition-all ease-in-out duration-200"
          >
        ...
        >
          <Command>
            ...
            />
            <CommandList>
              <CommandEmpty className="text-gray-500 text-xs">No se encontraron resultados.</CommandEmpty>
              <CommandGroup>
                {options.map((option) => (
                  <CommandItem
                    key={option.value}
                    ...
                    className={cn(
                      "flex items-center px-3 py-2 text-sm text-gray-700 cursor-pointer transition-all ease-in-out",
                      value === option.value
